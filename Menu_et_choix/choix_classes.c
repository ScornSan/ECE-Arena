...
void affichage_classe(t_joueur* joueur, BITMAP* buffer, BITMAP *bg, BITMAP* classe[], int *nb,  FONT* myfont, FONT* myfont2, BITMAP* desc_classe[])
{
    int x_depart = 40;
    char texte[50];
...
void choix_classe(BITMAP *buffer, BITMAP *fond, BITMAP* cursor, t_joueur* joueur, int nb_joueurs,  FONT* myfont, FONT* myfont2, SAMPLE* son_menu, BITMAP* son_on, BITMAP* son_off)
{
    ...
    {
        clear_bitmap(buffer);
        affichage_classe(joueur, buffer, fond, classe, &nb, myfont, myfont2, desc_classe);
