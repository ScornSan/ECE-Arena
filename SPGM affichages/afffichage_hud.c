...
/*
void affichage_hud_joueur_autre(BITMAP* buffer, BITMAP** hud, t_joueur* joueur, int i, BITMAP **icone_classes)
{
    if (joueur[i].pv <= 55)
    {
        rectfill(buffer, 72, 650, 74 + joueur[i].pv * 3, 673, makecol(255, 0, 0)); // rectangle pv a reduire quand il perd des hp
...
{
    rectfill(buffer, 1047, 663, 1257 ,712 , makecol(58, 43, 0));
    textprintf_ex(buffer, font, 1108 , 680 , makecol(0, 0, 0), -1, "FIN DE TOUR");
    if ( mouse_x>= 1047 && mouse_x <= 1257 && mouse_y >= 663 && mouse_y <= 712)
    {
        textprintf_ex(buffer,font,1108,680, makecol(255,255,255),-1,"FIN DE TOUR");
    }
}
