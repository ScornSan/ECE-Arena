                ...
                int *colonne_souris, int nb_joueurs, BITMAP * hud_icone, BITMAP * desc_sorts, BITMAP **hud_joueur, BITMAP **icone_classes, int respiration[])
{
    ...
    while(sort_lance == 0 && annulation == 0)
    {
        ...
        {
            chance = 8/*random(4,8)*/;
            if (pourcentage_de_chance() < 6 && joueur[i].pa >= 5 && chance)
            {
                sort_lance = 1;
                id_ennemi = matrice[*ligne_souris][*colonne_souris].occuper - 1;
            }
        }
    ...
    if (sort_lance == 1)
    {
        ...
        while((int)(time(NULL) - start < 4))
        {
            ...
            if (compteur == 40)
            {
                ...
                if (*ligne_souris <= joueur[i].x && *colonne_souris < joueur[i].y)
                {
                    //animation_attaque_de_base(buffer, bg, j, i, matrice, nb_joueurs, direction, respiration_joueur);
                    draw_sprite(buffer, joueur[i].classe.spell[3].anim_attaque_cible[1][num_sprites], matrice[joueur[i].x][joueur[i].y].x_bloc - 22, matrice[joueur[i].x][joueur[i].y].y_bloc - 52);
                    affichage_joueurs_respiration(buffer, joueur, i, nb_joueurs, matrice, respiration, i);
                }
                /// SUD EST
                if (*ligne_souris > joueur[i].x && *colonne_souris <=joueur[i].y)
                {
                    draw_sprite(buffer, joueur[i].classe.spell[3].anim_attaque_cible[3][num_sprites], matrice[joueur[i].x][joueur[i].y].x_bloc - 22, matrice[joueur[i].x][joueur[i].y].y_bloc - 52);
                    affichage_joueurs_respiration(buffer, joueur, i, nb_joueurs, matrice, respiration, i);
                }
                /// SUD OUEST
                if (*ligne_souris <= joueur[i].x && *colonne_souris < joueur[i].y)
                {
                    draw_sprite(buffer, joueur[i].classe.spell[3].anim_attaque_cible[0][num_sprites], matrice[joueur[i].x][joueur[i].y].x_bloc - 22, matrice[joueur[i].x][joueur[i].y].y_bloc - 52);
                    affichage_joueurs_respiration(buffer, joueur, i, nb_joueurs, matrice, respiration, i);
                }
                /// NORD EST
                if (*ligne_souris == joueur[i].x && *colonne_souris > joueur[i].y)
                {
                    draw_sprite(buffer, joueur[i].classe.spell[3].anim_attaque_cible[2][num_sprites], matrice[joueur[i].x][joueur[i].y].x_bloc - 22, matrice[joueur[i].x][joueur[i].y].y_bloc - 52);
                    affichage_joueurs_respiration(buffer, joueur, i, nb_joueurs, matrice, respiration, i);
                }
