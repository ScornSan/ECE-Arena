...
void sort2_assassin(time_t start, t_joueur* joueur, int i, int nb_joueurs, BITMAP *buffer, BITMAP* buffer_map, t_bloc matrice[23][23], int x_souris, int y_souris, BITMAP *cursor, BITMAP *map, BITMAP **hud_joueur, BITMAP **icone_classes, BITMAP *hud_icone, BITMAP *desc_sorts)
{
    ...
    {
        clear_bitmap(buffer);
        textprintf_ex(buffer, font, 0, 20, makecol(0, 0, 255), -1, "Temps: %f", (float)((time(NULL)) - start));
        blit(buffer, screen, 0, 0, 0, 0, SCREEN_W, SCREEN_H);
    }*/
    while(attaque == 3 || mess == 0)
    {
        ...
        if(attaque == 1)
        {
            ...
            {
                printf("joueur +X_ = %d, joeuur + y_a = %d     ", matrice[joueur[i].x][joueur[i].y].x_bloc + compteur*x_augmente, matrice[joueur[i].x][joueur[i].y].x_bloc + compteur*y_augmente);
                printf("souris x = %d, souris y = %d \n", matrice[x_temp][y_temp].x_bloc, matrice[x_temp][y_temp].y_bloc);
                compteur++;
            ...
            textout_ex(buffer, font,"ATTAQUE REUSSIE !!",15, 30, makecol(255,0,0), -1);
        ...
        {
            textout_ex(buffer, font,"ATTAQUE MANQUEE !!",15, 30, makecol(255,0,0), -1);
            if(temps +3 == (float)((time(NULL)) - start))
            {
                mess = 1;
            }
