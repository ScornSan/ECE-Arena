...
void sort3_vampire(t_joueur* joueur, int i, int nb_joueurs, BITMAP *buffer, BITMAP* buffer_map, t_bloc matrice[23][23], int x_souris, int y_souris, BITMAP *cursor, BITMAP *map, BITMAP **hud_joueur, BITMAP **icone_classes, BITMAP *hud_icone, BITMAP *desc_sorts)
{
    ...
    while (!attaque)
    {
        clear_bitmap(buffer);
        affichage_general(buffer, map, joueur, i, nb_joueurs, hud_joueur, icone_classes, hud_icone, desc_sorts);
        dessin_ligne(joueur, i, nb_joueurs, buffer, buffer_pixels, matrice, x_souris, y_souris, cursor, map, hud_joueur, icone_classes, hud_icone, desc_sorts, 3);
        dessin_diagonales(joueur, i, nb_joueurs, buffer, buffer_pixels, matrice, x_souris, y_souris, cursor, map, hud_joueur, icone_classes, hud_icone, desc_sorts, 3);
        dessin_bloc_unique(buffer_pixels, joueur[i].x + 2, joueur[i].y + 1, matrice, 0, 140, 0);
        dessin_bloc_unique(buffer_pixels, joueur[i].x - 2, joueur[i].y - 1, matrice, 0, 140, 0);
        dessin_bloc_unique(buffer_pixels, joueur[i].x - 2, joueur[i].y + 1, matrice, 0, 140, 0);
        dessin_bloc_unique(buffer_pixels, joueur[i].x + 2, joueur[i].y - 1, matrice, 0, 140, 0);
        dessin_bloc_unique(buffer_pixels, joueur[i].x + 3, joueur[i].y - 1, matrice, 0, 140, 0);
        dessin_bloc_unique(buffer_pixels, joueur[i].x - 3, joueur[i].y - 1, matrice, 0, 140, 0);
        affichage_joueurs(buffer, joueur, i, nb_joueurs, matrice);
        display_cursor(cursor, buffer, mouse_x - 5, mouse_y - 5);
        blit(buffer, screen, 0, 0, 0, 0, SCREEN_W, SCREEN_H);
