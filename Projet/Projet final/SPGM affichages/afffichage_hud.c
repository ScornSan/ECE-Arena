...
void affichage_fin_de_tour(BITMAP* buffer, t_joueur* joueur)
{
    ...
    draw_sprite(buffer, joueur[0].barre_temps, 1025, 650);
...
void affichage_hud_joueur(BITMAP* buffer, BITMAP** hud, t_joueur* joueur, int i, BITMAP **icone_classes)
{
    ...
    if (joueur[i].pa >= 6)
        rectfill(buffer, 68, 675, 70 + 180, 685, makecol(0, 0, 255)); // rectangle pv a reduire quand il perd des hp
    else
        rectfill(buffer, 68, 675, 70 + joueur[i].pa * 30, 685, makecol(0, 0, 255)); // rectangle pa a reduire quand il perd des hp

    if (joueur[i].pm > 3)
        rectfill(buffer, 66, 685, 70 + 180, 695, makecol(0, 255, 0)); // rectangle pv a reduire quand il perd des hp
    else
